odoo.define('website.s_website_form', function (require) {
    ...
    publicWidget.registry.s_website_form = publicWidget.Widget.extend({
        ...
        send: async function (e) {
            ...
            const emailField = this.$target.find('input[name="email_from"]'); // Select email field
            const email = emailField.val().trim();
            const apiKey = '0ba8478db7a65a00dfd3da37748540ed'; // Your Mailboxlayer API key

            if (!email) {
                self.update_status('error', _t("Email field cannot be empty."));
                $button.removeClass('disabled').removeAttr('disabled'); // Re-enable button
                emailField.focus();
                return false;
            }
            ...
            try {
                ...
                const apiUrl = `http://apilayer.net/api/check?access_key=${encodeURIComponent(apiKey)}&email=${encodeURIComponent(email)}`;
                const response = await fetch(apiUrl);
                const result = await response.json();

                if (!result.format_valid) {
                    self.update_status('error', _t("Invalid email format. Please enter a valid email address."));
                    $button.removeClass('disabled').removeAttr('disabled'); // Re-enable button
                    emailField.focus();
                    return false;
                }
                ...
                if (!result.smtp_check) {
                    self.update_status('error', _t("Entered email does not exist. Please check and try again."));
                    $button.removeClass('disabled').removeAttr('disabled'); // Re-enable button
                    emailField.focus();
                    return false;
                }
                ...
                self.update_status('error', _t("An error occurred while validating the email. Please try again later."));
                $button.removeClass('disabled').removeAttr('disabled'); // Re-enable button
                return false;
            }
